// from https://github.com/sahaj-b/ghostty-cursor-shaders/blob/main/ripple_cursor.glsl
// CONFIGURATION
const float DURATION = 0.15;               // How long the ripple animates (seconds)
const float MAX_RADIUS = 0.15;             // Max radius in normalized coords (0.5 = 1/4 screen height)
...
vec4 COLOR = vec4(0.95, 0.25, 0.74, 0.8);
...
vec3 aurora(float t) {
    vec3 c1 = vec3(0.18, 0.86, 0.96);
    vec3 c2 = vec3(0.34, 0.78, 0.97);
    vec3 c3 = vec3(0.63, 0.40, 0.91);
    vec3 c4 = vec3(0.95, 0.25, 0.74);

    float phase = clamp(t, 0.0, 1.0);
    float blend1 = smoothstep(0.00, 0.20, phase);
    float blend2 = smoothstep(0.18, 0.62, phase);
    float blend3 = smoothstep(0.55, 1.00, phase);

    vec3 color = mix(c1, c2, blend1);
    color = mix(color, c3, blend2);
    color = mix(color, c4, blend3);

    return mix(color, vec3(0.88, 0.82, 0.96), 0.04);
}
...
void mainImage(out vec4 fragColor, in vec2 fragCoord){
    ...
    // don't clamp yet; we need to know if it's > 1.0 (finished)
     ...
     if (isModeChange > 0.0 && isAnimating > 0.0) {
        ...
        float angularPhase = atan(vu.y - centerCC.y, vu.x - centerCC.x) / (2.0 * PI);
        float gradientPhase = clamp(rippleProgress * 0.75 + angularPhase * 0.18 + 0.35, 0.0, 1.0);
        vec3 auroraRgb = aurora(gradientPhase);
        vec4 rippleColor = vec4(auroraRgb, COLOR.a);

        // Apply ripple effect
        fragColor = mix(fragColor, rippleColor, ripple * rippleColor.a);
    }
